...
"2025-01-21",40,45`;
    ...
    const localReaderPromise: Promise<Record<string,string | number>[]> = localReader(path);
    ...
    const localReaderData: Record<string,string | number>[] = await localReaderPromise;
  ...
  test('translating timeSeriesData to and from', async () => {
    ...
    const line4: Record<string,string | number> = {Time: '2025-01-07', X: 16, Y: 35};
