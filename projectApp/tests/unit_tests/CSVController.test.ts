...
describe("CSVController Tests", () => {
  ...
  test("browserCSVFiles should return the tuple from loadedCsvBrowser", () => {
    const fakeBrowserData: [string, boolean][] = [["file.csv", true]];
    // Force the model's loadedCsvBrowser to return the fake data.
    vi.spyOn(csvController.getModel(), "loadedCsvBrowser").mockReturnValue(
      fakeBrowserData,
    );
    expect(csvController.browserCSVFiles()).toEqual(fakeBrowserData);
  });
  ...
  test("generate should create graphs and push them to the main controller", () => {
    ...
    expect(emSetNameSpy).toHaveBeenCalledWith("TestGraph");
